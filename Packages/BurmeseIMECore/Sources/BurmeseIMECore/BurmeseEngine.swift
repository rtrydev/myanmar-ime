...
public final class BurmeseEngine: @unchecked Sendable {
    ...
    private static let lmPruneMargin: Double = 8.0
    ...
    private static let anchorCommitThreshold = 8

    /// Page size for candidate display.
    public static let candidatePageSize = 9
    ...
    private func renderFrozenPrefixBranches(_ prefix: String, baseContext: [String]) -> [FrozenPrefixBranch] {
        ...
        if parses.isEmpty {
            ...
            if let topScore = scored.first?.lmScore {
                scored = scored.filter { topScore - $0.lmScore <= Self.lmPruneMargin }
            }
    ...
    public func update(buffer: String, context: [String]) -> CompositionState {
        ...
        let priorAnchor: PrefixAnchor? = {
            for anchor in historySnapshot.reversed() {
                guard !anchor.normalized.isEmpty,
                      ...
                      anchor.normalized.count >= Self.anchorCommitThreshold
                else { continue }
        ...
        grammarCandidates = Self.pruneByLmMargin(grammarCandidates, keyPath: \.lmLogProb)
        ...
        uniqueLexiconCandidates = Self.pruneByLmMargin(uniqueLexiconCandidates, keyPath: \.lmLogProb)
    ...
    private static func pruneByLmMargin<T>(
        _ candidates: [T],
