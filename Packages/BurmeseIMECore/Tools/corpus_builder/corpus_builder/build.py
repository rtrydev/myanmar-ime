...
def _default_parser() -> argparse.ArgumentParser:
    ...
    for name in ("ingest", "vocab", "lexicon", "lm", "all"):
        ...
        # Aggressive default prune: keep all unigrams, drop bigrams seen
        # at most twice and trigrams seen at most thrice. The earlier
        # `[0,0,1]` produced a ~900 MB .bin dominated by long-tail
        # trigrams that contribute almost nothing over their bigram
        # backoff. `[0,2,3]` typically lands in the 20–50 MB range while
        # preserving the high-frequency continuations the IME ranker
        # actually needs.
        sp.add_argument("--prune", type=int, nargs="+", default=[0, 2, 3])
        sp.add_argument(
            "--workers",
            type=int,
...
def cmd_ingest(args: argparse.Namespace) -> None:
    ...
    cfg = ingest.IngestConfig(corpus=args.corpus, max_docs=args.max_docs)
...
def cmd_lexicon(args: argparse.Namespace) -> None:
    ...
    counts = _load_counts(args.work_dir)
    curated = vocab.read_curated_tsv(args.curated_tsv)
    rows = lexicon.write_tsv(args.tsv_out, v, counts, curated)
